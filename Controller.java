   ...
   @PostMapping("/uploadFile")
    ...
    responseHeaders.setContentType(MediaType.TEXT_HTML);
        try {
            ...
            Idcard dbFile = new Idcard(username,file.getBytes(),fileName,file.getContentType());
        ...
        String fileDownloadUri = ServletUriComponentsBuilder.fromCurrentContextPath()
            ...
            .path(image.filename)
            .toUriString();
          output = "<script>window.location.replace(\"/studentapply\")</script>";
        } catch (Exception ex) {
    ...
    @ResponseBody
    void image(@PathVariable String username, HttpServletResponse response) throws Exception  {
        // Load file as Resource
        List<Idcard> databaseFile = idcardRepository.findByUsername(username);

        response.setContentType("image/jpeg, image/jpg, image/png, image/gif");
		response.getOutputStream().write(databaseFile.get(0).file);
